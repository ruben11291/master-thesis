...
for host in hosts:
	...
	nodes.append(node)
	command_client = "ping %s -w %d  > node%d.out " % (bonfire_host[1],seconds,node)
...
#print "ENDDDDDDDD"
#asd=("France","Spain","Germany","China","Thailand","Austria","Australia","Brazil","Canada")#,"UnitedStates","RussianFederation")
#for host in asd:
#	print host
#	node= create_node2(ec,slice,pleuser,plepass,country=host)
#	nodes.append(node)
#	command_client = "ping %s -w %d  > node%d.out " % (bonfire_host[1],seconds,node)
#	app=create_app(ec,command_client)
#	ec.register_connection(app,node)
