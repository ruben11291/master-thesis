...
class GroundStation():
    ...
    def __init__(self,id,scenario,hostdb,orchestrator_prx):
       ...
       # self.handlerClass = ConnectionHandler
	    ...
	    #pdb.set_trace()
            ...
            self.serverFunction(orchestrator_prx)
    ...
    def serverFunction(self,orchestrator_prx):
        ...
        while True:
            ...
            for sock in sread:
                if sock == self.created_socket:
                    ...
                    self.pid = os.fork()
                    if self.pid == 0:          
                        self.Download(newsock,orchestrator_prx)
                    else:
                        self.pids.append(self.pid)
            

            
    def Download(self,sock,orches_prx):
        try:
            self.socket = sock
            self.orchestrator=orches_prx
    ...
    def createFile(self,satellite, useless_images, usefull_images):
	    ...
	    # os.system("sudo chmod 777  /tmp/"+name+".bin")
            ...
            ami=self.orchestrator.begin_downloadedImage("/tmp/"+name+".bin")
            ami.waitForSent()
...
if __name__=="__main__":
    ...
    try:
        ...
        logger.info("[GroundStation%s] The orchestrator is not available"%(self.id))
        sys.exit(1)
    ...
    scenario = sys.argv[3]
    host = sys.argv[4]
    gs = GroundStation(id,scenario,host,orches)
